...
#define enc28j60_select()  	SPI3_NSS_GPIO_Port->BSRR = (uint32_t)SPI3_NSS_Pin << 16U; DWT_Delay_us(5) /* HAL_GPIO_WritePin(SPI3_NSS_GPIO_Port, SPI3_NSS_Pin, GPIO_PIN_RESET); HAL_Delay(1) */
#define enc28j60_release() 	SPI3_NSS_GPIO_Port->BSRR = SPI3_NSS_Pin; DWT_Delay_us(5) /* HAL_GPIO_WritePin(SPI3_NSS_GPIO_Port, SPI3_NSS_Pin, GPIO_PIN_SET); HAL_Delay(1) */
...
uint8_t enc28j60_rxtx(uint8_t data)
{
	...
	if(HAL_SPI_TransmitReceive(&hspi3,(uint8_t*) &data,(uint8_t*) &receivedbyte,1,0x2000)!=HAL_OK)
	{
		Error();
	}
...
void enc28j60_soft_reset()
{
	...
	//HAL_Delay(20); // Wait until device initializes
}
...
void enc28j60_init(uint8_t *macadr)
{
	...
	// Reset ENC28J60
	enc28j60_soft_reset();
	//while(!enc28j60_read_op(0x00,ESTAT)&ESTAT_CLKRDY);
	// Setup Rx/Tx buffer
...
static void Error (void)
{
	/* When it is initialization error for ENC28J60 */
